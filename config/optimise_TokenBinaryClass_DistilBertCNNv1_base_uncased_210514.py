...
from model.DistilBertConv2dForTokenClassification import DistilBertCNNForTokenClassification
from model.DistilBertCNNConfig import DistilBertCNNConfig

from pipeline.pipeline_tokenclassifier import TokenClassifierPipeline
from utils.objdict import ObjDict

# __________________________________________________________________ ||
name = "optimise_TokenBinaryClass_DistilBertCNNv1_base_uncased_210514"
base_pretrained = "distilbert-base-uncased"
preprocess_train_dir = "data/optimise_tokenclassification_210514_01_hasDataset/train/" 
preprocess_test_dir = "data/optimise_tokenclassification_210514_01_hasDataset/test/" 
label_list = [0,1,]

# __________________________________________________________________ ||
pipeline = TokenClassifierPipeline()

model_config = DistilBertCNNConfig(
        in_channels = 1,
        out_channels = 1,
        conv_kernel_size=(16,4),
...
model = DistilBertCNNForTokenClassification(model_config)
#tokenizer = DistilBertTokenizer.from_pretrained('tokenizer/'+base_pretrained)
tokenizer = DistilBertTokenizer.from_pretrained(base_pretrained)
...
evaluate_cfg = ObjDict(
        pretrain_model = os.path.join("log",name,"checkpoint-7000"),
        device = 'cpu',
        batch_size = 256,
        print_per_step = 1,
        test = True,
...
extract_cfg = ObjDict(
        ...
        #test = False,
        #write_per_step = 100,
        #extract_text_path = os.path.join('tmp',name+'_extract_train_7000.txt'),
        test = True,
        write_per_step = 1,
